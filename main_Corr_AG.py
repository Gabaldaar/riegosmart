...
wlan = network.WLAN(network.STA_IF) # Instancia global única
...
async def conectar_wifi():
    global wifi_conectado, ssid_configurado, wlan
    try:
    ...
    wlan.active(True)
    wlan.disconnect() # Limpiar estado interno
    await asyncio.sleep(0.5)
    wlan.connect(ssid, password)
    
    for _ in range(50): # Timeout extendido a 25s
        if wlan.isconnected():
            wifi_conectado = True
            print("Wi-Fi Conectado.")
            return True
        estado = wlan.status()
        if estado == network.STAT_WRONG_PASSWORD:
            print("Error: Contraseña Wi-Fi incorrecta.")
            break
        elif estado == network.STAT_NO_AP_FOUND:
            print("Error: Red Wi-Fi no encontrada.")
            break
        await asyncio.sleep(0.5)
        
    print("Timeout o Error de Wi-Fi.")
    # No apagamos la interfaz para evitar inestabilidad del módem
    return False

intentos_reconex = 0
async def mantener_conexion_wifi():
    global wifi_conectado, mqtt_client, wlan, intentos_reconex
    while True:
        ...
        espera = 10 if intentos_reconex < 3 else 60
