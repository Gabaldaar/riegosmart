# main.py - Lógica del dosificador con Control Proactivo de Bomba (Híbrido Nube/BLE)
import time
...
def actualizar_led():
    ...
    patron_seleccionado = f"{base_patron}_refuerzo" if Refuerzo == 1 else base_patron
    if patron_seleccionado not in LED_PATRONES: patron_seleccionado = 'inactivo'
        
    if estado_led_actual['patron'] != LED_PATRONES[patron_seleccionado]:
        estado_led_actual = {'patron': LED_PATRONES[patron_seleccionado], 'indice': 0, 'ultimo_cambio': time.time(), 'estado': 0}
        ref.value(0)
