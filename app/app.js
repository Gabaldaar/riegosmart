...
let mqttClient = null;
let sysLogsArr = [];
let unsubHistorial = null; // Lo dejamos si Firestore todavía se usa para historial, o lo quitamos si va por telemetría.
...
function setConexionModo(modo) {
    ...
    if (modo === "NUBE") {
        ...
        lblConnMode.innerText = "OFFLINE (Toca para BLE)";
    ...
    if (!lblConnMode.dataset.clickEvent) {
        lblConnMode.addEventListener('click', () => {
            if (modoConexion === "OFFLINE") {
                document.getElementById('btnForceBLE').click();
            }
        });
        lblConnMode.dataset.clickEvent = "true";
    }
...
function connectNube() {
    ...
    mqttClient.onMessageArrived = (message) => {
        ...
        try {
            ...
            if (topic === `dosimat/${currentMac}/telemetria`) {
                ...
                sysLogsArr.unshift(`${data.fecha || ""} - ${data.evento || ""}`);
                if (sysLogsArr.length > 100) sysLogsArr.pop();
                renderSysLog(sysLogsArr);
            }
        } catch (e) {
    ...
    const options = {
        ...
        onSuccess: () => {
            ...
            mqttClient.subscribe(`dosimat/${currentMac}/telemetria`);
            mqttClient.subscribe(`dosimat/${currentMac}/sys_log`);
            sysLogsArr = []; 
            if (modoConexion !== "BLE") {
                setConexionModo("NUBE");
                // Enviar ping silencioso para forzar al ESP32 a responder con telemetría
                sendCommand({comando: "pedir_historial"}, true);
            }
        },
        onFailure: (err) => {
...
async function sendCommand(obj, silent = false) {
    if (!silent) showToast("Enviando orden al dispositivo...", true);
        ...
        while (retries > 0) {
            try {
                for (let i = 0; i < data.length; i += 20) {
                    ...
                    await new Promise(r => setTimeout(r, 60)); // Reducido levemente
                }
                return true; // Éxito
            } catch (error) {
                console.error(`BLE Send Error (Retries left: ${retries-1}):`, error);
                retries--;
                if (retries === 0) {
                    if (!silent) showToast("Error al enviar comando por BLE", true);
                    return false;
                }
                await new Promise(r => setTimeout(r, 500)); // Esperar a que el radio Wi-Fi se libere
            }
        }
    } else if ((modoConexion === "NUBE" || modoConexion === "OFFLINE") && currentMac) {
        try {
            if (mqttClient && mqttClient.isConnected()) {
                const topic = `dosimat/${currentMac}/comandos`;
                const message = new Paho.MQTT.Message(JSON.stringify(obj));
                message.destinationName = topic;
                mqttClient.send(message);
                console.log("Comando enviado a la Nube vía MQTT:", obj.comando);
                
                if (window.cmdTimeout) clearTimeout(window.cmdTimeout);
                window.cmdTimeout = setTimeout(() => {
                    setConexionModo("OFFLINE");
                    if (!silent) showToast("El equipo no responde. Parece estar APAGADO.", true);
                    window.cmdTimeout = null;
                }, 15000); // 15 segundos de tolerancia
            } else {
                throw new Error("MQTT desconectado");
            }
        } catch(e) {
...
function renderHistorial(historialArray) {
    ...
    if (!historialArray || historialArray.length === 0) {
        ...
        historialArray.forEach(h => {
            ...
            if (isPerdida || isAtasco) tr.className = "fila-perdida";
...
document.getElementById('btnGuardarCronograma').onclick = async () => {
    ...
    document.querySelectorAll('.crono-row').forEach(fila => {
        ...
        if(timeInput && !isNaN(durInput) && durInput > 0) {
            cronograma.push({
                on: timeInput, 
                duracion: durInput, 
    ...
    if (filasIncompletas) {
        if (!await customConfirm("Hay filas en el cronograma con la Hora o los Minutos en blanco/cero. Éstas serán ignoradas al guardar. ¿Deseas continuar?", "Atención")) {
            return;
        }
    }

    if (modoConexion === "BLE") {
        // Protocolo chunked: enviamos el cronograma entrada por entrada para
        // evitar que un JSON grande (~600 bytes) se pierda en la fragmentación BLE.
        // cron_start → N × cron_add → cron_commit
        showToast(`Enviando ${cronograma.length} programas por BLE...`);
        await sendCommand({comando: "cron_start", total: cronograma.length}, true);
        ...
        for (let i = 0; i < cronograma.length; i++) {
            await sendCommand({
                ...
                idx:      i,
                on:       cronograma[i].on,
                duracion: cronograma[i].duracion,
                dosis:    cronograma[i].dosis,
                dias:     cronograma[i].dias
            }, true);
            await new Promise(r => setTimeout(r, 300));
        }
        await sendCommand({comando: "cron_commit"});
    } else {
        // WiFi/NUBE: JSON completo — simple.py garantiza lectura correcta del payload TCP
